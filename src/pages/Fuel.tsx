...
const FuelPage: React.FC = () => {
  ...
  const filteredFuelRecords = mockFuelRecords.filter((record) => {
    const matchesSearch =
      searchTerm === "" ||
    ...
    const matchesFuelType =
      filterFuelType === "" || record.fuelType === filterFuelType;

    const matchesVehicle =
      filterVehicle === "" || record.vehicleId === filterVehicle;
  ...
  const handleDelete = (id: string) => {
    if (
      ...
      console.log("Delete fuel record:", id);
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Gérez et suivez toute la consommation de carburant de votre flotte
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Total Pleins</p>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Prix Moyen/L</p>
              <p className="text-2xl font-bold text-gray-900">
              ...
              {filteredFuelRecords.length === 0 ? (
                ...
                filteredFuelRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {(record.cost / record.quantity).toFixed(2)} TND
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {record.cost.toFixed(2)} TND
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        onClick={() => navigate(`/fuel/${record.id}`)}
